...
function checkText(text, opts) {
  ...
  // A register may switch off an invariant that does not apply to its artifact.
  // The rule still runs and still reports; it just stops gating.
  const registerEntry = (config.registers.registers || {})[register] || {};
  const disabled = new Set(registerEntry.disableInvariants || []);

  const invariantResults = runInvariants(doc, invariants, vocab, lang);
  for (const result of invariantResults) {
    if (disabled.has(result.id)) {
      result.gates = false;
      result.disabledBy = register;
    }
  }

  const results = [...invariantResults, ...runRegister(doc, register, config.registers)];
